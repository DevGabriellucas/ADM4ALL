...
export const CronogramaList = ({ turmaId, aulas }: CronogramaListProps) => {
  ...
  const enviar = () => {
    ...
    if (titulo.trim() === "") {
      setFeedback({ tipo: "erro", texto: "Informe o título da aula." });
      return;
    }
  ...
  const atualizar = (aula: AulaResumo, input: AtualizarAulaInput) => {
    if (!turmaId) return;

    setAtualizandoId(aula.id);
    startAtualizacao(async () => {
      const resultado = await atualizarAulaAction(turmaId, aula.id, input);
  ...
  const editar = (aula: AulaResumo) => {
    const novoTitulo = window.prompt("Título da aula", aula.titulo);
        ...
        <ConfirmDialog
          ...
          description={
            confirmacao.tipo === "remover"
              ? `A aula "${confirmacao.aula.titulo}" será removida do cronograma. Essa ação não pode ser desfeita.`
              : `A aula "${confirmacao.aula.titulo}" ficará marcada como cancelada no cronograma.`
          }
          confirmLabel={
            confirmacao.tipo === "remover" ? "Remover aula" : "Cancelar aula"
          }
