...
const INITIAL_FORM_DATA: CoordinatorFormData = {
  nome: "",
  email: "",
  cpf: "",
  telefone: "",
...
export const NewCoordinatorForm = ({
  isOpen,
  onCancel,
}: NewCoordinatorFormProps) => {
  const router = useRouter();
  const [formData, setFormData] =
    useState<CoordinatorFormData>(INITIAL_FORM_DATA);
  ...
  const handleSubmit = async (event: SubmitEvent<HTMLFormElement>) => {
    ...
    const resultado = await convidarCoordenadorAction({
      nome: formData.nome,
      email: formData.email,
      cpf: formData.cpf,
      telefone: formData.telefone || undefined,
      areaCoordenacao: formData.areaCoordenacao || undefined,
    });
      ...
      <form onSubmit={handleSubmit} className="mt-5">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <label className="flex flex-col gap-y-2 font-medium text-slate-700 text-sm md:col-span-2">
            ...
            <input
              type="text"
              value={formData.areaCoordenacao}
              onChange={(event) =>
                setFormData({
                  ...formData,
                  areaCoordenacao: event.target.value,
                })
              }
              placeholder="Ex: Gestão de Turmas (opcional)"
              className="h-11 rounded-lg border border-slate-300 bg-white px-3 font-normal text-slate-900 outline-none transition-colors placeholder:text-slate-400 focus:border-brand-medium focus:ring-2 focus:ring-brand-light/30"
            />
          </label>
        </div>
